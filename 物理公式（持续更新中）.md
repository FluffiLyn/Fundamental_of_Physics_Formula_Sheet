...
5、带电圆环的电场强度 $E=\displaystyle\frac{1}{4\pi\varepsilon_0}\cdot\frac{qz}{(z^2+R^2)^{3/2}}$
...
6、带电圆盘的电场强度 $E=\displaystyle\frac{\sigma}{2\varepsilon_0}(1-\frac{z}{\sqrt{z^2+R^2}})$
* 推导方法：将圆盘分为无穷多个圆环， $dq=\sigma dA=\sigma 2\pi rdr$

7、电偶极子的扭矩 $\vec{\tau}=\vec{p}\times\vec{E}$

8、电偶极子的电势能 $U=-\vec{p}\cdot\vec{E}$
...
4、线电荷在r处的电场 $E=\displaystyle\frac{\lambda}{2\pi r\varepsilon_0}$
...
* 电场强度 $E=\displaystyle\frac{\sigma}{\varepsilon_0\kappa}$
...
两者等价，选其一记忆即可。其中 $\Phi$ 或 $W_0$ 是逸出功

3、光子动量 $p=\displaystyle\frac{h}{\lambda}$
* 光没有静止质量，但是有动量

4、康普顿效应 $\Delta\lambda=\displaystyle\frac{h}{mc}(1-\cos\phi)$
* 建议直接背，因为推导过程很复杂

5、物质波的德布罗意波长 $\lambda=\displaystyle\frac{h}{p}$

6、波动方程 $\Psi(x,y,z,t)=\psi(x,y,z)e^{-i\omega t}$
* $\omega$ 是角频率， $\omega=2\pi f$
* $|\psi|^2$ 是波函数的概率密度
* $\Psi^*$是波函数的共轭复数，将i变为-i

7、一维运动薛定谔方程 $\displaystyle\frac{d^2\psi}{dx^2}+\frac{8\pi^2m}{\hbar^2}[E-U(x)]\psi=0$
* 这真的会考吗？

8、自由粒子的薛定谔方程 $\displaystyle\frac{d^2\psi}{dx^2}+k^2\psi=0$
* 其中 $k^2=\displaystyle\frac{8\pi^2mE}{h^2}$
* 不会考吧

9、海森堡不确定性原理（测不准原理）
$$\begin{cases}
\Delta x\Delta p\geq\displaystyle\frac{\hbar}{2} \\\\
\Delta y\Delta p\geq\displaystyle\frac{\hbar}{2} \\\\
\Delta z\Delta p\geq\displaystyle\frac{\hbar}{2}
\end{cases}$$
* 其中 $\hbar=\displaystyle\frac{h}{2\pi}$ 是约化普朗克常数

10、势垒贯穿透射系数 $T=e^{-2b x}$
